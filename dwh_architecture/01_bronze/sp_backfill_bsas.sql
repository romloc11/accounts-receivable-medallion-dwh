/* ============================================================================
   bronze.backfill_bsas
   Purpose : Historical load of bronze.sap_bsas (cash accounts 111xxx/113xxx),
             one posting year at a time.
   Run     : EXEC bronze.backfill_bsas @start_year = 2022 [, @end_year = 2026];
             On an empty table, before the first bronze.load_bronze, outside
             business hours (one scan of P01 BSAS per year). Resumable: a year
             that already has rows in the target is skipped.
   Notes   : Positional INSERT ... SELECT *: bronze.sap_bsas keeps P01's column
...
BEGIN
    ...
    DECLARE @proc VARCHAR(128) = 'bronze.backfill_bsas',
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            SET @step = 'bronze.sap_bsas ' + CAST(@yr AS VARCHAR(4)); SET @t = SYSDATETIME();
