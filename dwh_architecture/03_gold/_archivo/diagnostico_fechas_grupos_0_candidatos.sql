...
-- Los CP/SA que SI aparecen en los grupos "0 candidatos disponibles" no
-- entraron a #aplicacion en el poblado real (que solo mira fecha_compensacion
-- DENTRO del rango jun-jul 2026, igual que #factura). Hipotesis: esos
-- documentos comparten documento_compensacion con la factura del rango, pero
-- su PROPIA fecha_compensacion cae FUERA de la ventana - el mismo fenomeno ya
-- documentado para el "virgen" (por eso #virgen NO se restringe por fecha).
-- Este script compara, factura por factura, la fecha_compensacion de la
-- factura contra la de los candidatos CP/SA/DZ de su mismo grupo.
...
residuo AS (
    SELECT f.sociedad, f.cliente_id, f.documento_compensacion, f.ejercicio_compensacion,
           f.documento_id, f.ejercicio, f.posicion, f.fecha_compensacion AS fecha_compensacion_factura,
...
grupos_0_candidatos AS (
    SELECT DISTINCT r.sociedad, r.cliente_id, r.documento_compensacion, r.ejercicio_compensacion, r.fecha_compensacion_factura
    FROM residuo r
    WHERE NOT EXISTS (
...
SELECT
    b.clase_documento,
    b.debe_haber,
    CASE WHEN b.fecha_compensacion BETWEEN @fecha_inicio AND @fecha_fin THEN 'DENTRO del rango' ELSE 'FUERA del rango' END AS fecha_compensacion_vs_rango,
    DATEDIFF(DAY, g.fecha_compensacion_factura, b.fecha_compensacion) AS dias_diferencia_vs_factura,
    COUNT(*) AS num_lineas,
...
WHERE b.clase_documento IN ('CP','SA','DZ','ZY','C5','D1','C1','C2','C3','C4','AB','Z1','Z2','Z3')
GROUP BY b.clase_documento, b.debe_haber,
    CASE WHEN b.fecha_compensacion BETWEEN @fecha_inicio AND @fecha_fin THEN 'DENTRO del rango' ELSE 'FUERA del rango' END,
    DATEDIFF(DAY, g.fecha_compensacion_factura, b.fecha_compensacion)
ORDER BY b.clase_documento, num_lineas DESC;
